...
import {CharityDistributorV2} from "./CharityDistributorV2.sol";
import {CreatorRouter} from "./CreatorRouter.sol";
import {CreatorRouterFactory} from "./CreatorRouterFactory.sol";
...
contract CharityLaunchpadV2 {
    ...
    event CreatorRouterDeployed(address indexed token, address indexed router);
    ...
    /**
     * Where an {CreatorRouter.Mode.XAccount} share is ring-fenced.
     *
     * ⚠ MAY be zero, and then a launch simply cannot choose that option — {CreatorRouter} refuses
     * to construct with an X share and no claims contract. That is deliberate: the claims contract
     * needs a signer that verifies X accounts, and a launchpad deployed before that exists should
     * offer the two options it can actually honour rather than a third it cannot.
     */
    address public immutable claims;

    /**
     * Who builds a {CreatorRouter}.
     *
     * ⛔⛔ A SEPARATE CONTRACT FOR A MEASURED REASON, NOT AN ABSTRACTION. Writing `new CreatorRouter`
     * here embeds the router's whole creation code in THIS contract's runtime bytecode, which put it
     * at 29,206 bytes against EIP-170's 24,576 — undeployable. See {CreatorRouterFactory}.
     */
     ...
     */
    /**
     * ⭐⭐ WHAT THE CREATOR'S HALF DOES, chosen at launch and fixed for ever.
     *
     * Empty means the V1 behaviour: the whole remainder is paid to `creatorPayout` as a plain
     * wallet, no router is deployed, and nothing about the launch costs more gas than it used to.
     * A launch that wants more than that supplies shares adding to 10,000 and gets a
     * {CreatorRouter}, whose address commits to them.
     */
    struct CharityTerms {
        address charity;
        bytes32 charityId;
    ...
    function launch(
        ...
        address creatorPayout,
        uint16 charityBps
    ) external payable returns (address token, address curve, address distributor, address router) {
        /* ⚠ The simple entry point takes no splits, so it is exactly the V1 launch: the remainder
           goes to the wallet and no router is deployed. `router` comes back zero, which is the
           honest answer rather than an address that does nothing. */
        CreatorRouter.Split[] memory none = new CreatorRouter.Split[](0);
        return _launch(
            params, launchConfigId, pairToken,
            CharityTerms({
                charity: charity, charityId: charityId, creatorPayout: creatorPayout,
                charityBps: charityBps, splits: none
            }),
    ...
    function _launch(
        ...
        DevBuy memory devBuy,
        address[] memory exemptions
    ) internal returns (address token, address curve, address distributor, address router) {
        ...
        */
        ...
        /*
          ⛔⛔ THE ORDER HERE IS FORCED BY THREE IMMUTABLES, AND IT ONLY GOES ONE WAY.

          The router must be the distributor's `opsVault`, which is fixed when the DISTRIBUTOR is
          built. The distributor must be the launch's `creatorFeeRecipient`, which is fixed when the
          LAUNCH is made. So: router, then distributor, then launch, then bind the router to the
          token that did not exist until the line before.

          ⭐ A launch with no splits gets NO ROUTER. The remainder goes to the wallet exactly as it
          did in V1 — same gas, same shape, one less contract to reason about. Paying for a splitter
          that splits one way would be a cost with nothing on the other side of it.
        */
        address payout = terms.creatorPayout;
        if (terms.splits.length > 0) {
            /* ⚠ A CALL, not a `new`. See {routerFactory} — the creation code has to live somewhere
               that is not this contract's own bytecode. */
            router = CreatorRouterFactory(routerFactory).deploy(
                address(factory), address(this), claims, address(poolManager), terms.splits
            );
            payout = router;
        }

        distributor = address(
            new CharityDistributorV2(
                escrow,
                terms.charity,
                payout,
                terms.charityBps,
        ...
        _distributorOf[token] = distributor;
        _launches.push(
            Entry({
                token: token,
                curve: curve,
        ...
        /* ⛔⛔ BOUND HERE, IN THE SAME TRANSACTION AS THE LAUNCH, AND THAT IS NOT A CONVENIENCE.
           `initialize` is one-shot and permissionless: a router left unbound between two blocks is
           one a stranger can bind to their own launch first, and it stays bound to it for ever.
           The router's own checks refuse that — see {CreatorRouter.initialize} — but leaving no
           window at all is the stronger guarantee, and it costs nothing to take.
           ⚠ AFTER `_distributorOf` is written, because that is one of the three reads it verifies. */
        if (router != address(0)) CreatorRouter(payable(router)).initialize(token);

        emit CharityLaunch(token, distributor, terms.charity, terms.charityId, msg.sender, terms.charityBps);
        if (router != address(0)) emit CreatorRouterDeployed(token, router);
    }
    ...
    /**
     * ⭐ The read {CreatorRouter.initialize} pins itself with. It answers only for tokens THIS
     * launchpad created, which is what an attacker cannot forge — and unlike {entryOf} it returns
     * zero rather than reverting, because a router checking it wants a comparison, not a throw.
     */
    mapping(address => address) private _distributorOf;

    function distributorOf(address token) external view returns (address) {
        return _distributorOf[token];
    }
